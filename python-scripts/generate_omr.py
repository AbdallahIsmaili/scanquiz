...
# Fill one random bubble for each question
for q, question in enumerate(questions):
    if q < 25:
        x_offset = left_margin
        y_offset = top_margin + q * question_spacing
    ...
    # Randomly select an option for this question
    random_option = random.randint(0, options_per_question - 1)
    center = (x_offset + random_option * bubble_spacing, y_offset)

    # Fill the selected bubble
    cv2.circle(omr_sheet, center, bubble_radius - 5, (0, 0, 0), -1)  # Fill the bubble
    log(f"Filled bubble for Q{q + 1}, Option {chr(65 + random_option)}")
